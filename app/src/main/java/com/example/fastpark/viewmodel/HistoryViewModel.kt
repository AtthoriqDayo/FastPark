...
class HistoryViewModel : ViewModel() {
    ...
    fun fetchParkingHistory() {
        viewModelScope.launch {
            ...
            if (userUid == null) {
                _uiState.value = HistoryUiState.Error("Pengguna tidak login.")
                return@launch
            }
            ...
            try {
                val querySnapshot = firestore.collection("parking_sessions")
                    ...
                    .whereEqualTo("status", "paid") // Hanya ambil sesi yang sudah selesai
                    .orderBy("exitTimestamp", Query.Direction.DESCENDING) // Urutkan dari yang terbaru
                    .get()
                    .await()
                ...
                _uiState.value = HistoryUiState.Error("Gagal memuat riwayat: ${e.message}")
            }
        }
    }
